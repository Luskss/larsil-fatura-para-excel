// LEITURA APENAS: efeito do agrupamento por maco (irmaos) e dos empates,
// nos 6 meses. Confere tambem o caso MACPONTA.
...
const valorOcr=o=>{const v=o&&o.valorTxt;if(v==null)return null;const n=Number(String(v).replace(/\./g,'').replace(',','.'));return isFinite(n)&&n>0?n:null;};

console.log('=== efeito do agrupamento por maco, por mes ===');
console.log('mes      pares  orfaosBRUTO  orfaosLIQ  irmaos  empatados');
let tb=0,tl=0,ti=0,te=0,tp=0;
const exIrmaos=[],exEmp=[];
for(const PER of MESES){
  ...
  const docsPorMes={};
  for(const off of [0,...P.VIZINHANCA]){const alvo=P.deslocarPeriodo(PER,off);
    docsPorMes[alvo]=(arquivosPorMes[alvo]||[]).map(a=>{const o=ocr[a.nome]||null;
      return P.enriquecerComOcr(P.documentoDoArquivo(a.nome,a.rel),
        o?{numero:o.numero,emitente:o.emitente,valor:valorOcr(o),dtEmissao:null}:null);});}
  const L=itens.map(P.lancamentoDaPlanilha);
  const r=P.conferirPeriodo(L,docsPorMes,PER);
  const np=r.pares.length+r.paresVizinhos.length;
  tp+=np;tb+=r.documentosSemLancamentoBruto;tl+=r.documentosSemLancamento;
  ti+=r.irmaosAgrupados;te+=r.empatados;
  console.log(`${PER}  ${String(np).padStart(5)}  ${String(r.documentosSemLancamentoBruto).padStart(11)}  ${String(r.documentosSemLancamento).padStart(9)}  ${String(r.irmaosAgrupados).padStart(6)}  ${String(r.empatados).padStart(9)}`);
  for(const p of [...r.pares,...r.paresVizinhos]){
    if(p.irmaos&&exIrmaos.length<10)exIrmaos.push(`${PER} NF ${p.lancamento.nf} ${p.lancamento.entidade.slice(0,24)}\n      par: ${p.documento.arquivo.slice(0,70)}\n      irmaos: ${p.irmaos.map(x=>x.arquivo.slice(0,60)).join(' | ')}`);
    if(p.empatado&&exEmp.length<8)exEmp.push(`${PER} NF ${p.lancamento.nf} ${p.lancamento.entidade.slice(0,24)} R$${p.lancamento.valor} forca ${p.forca}\n      escolhido: ${p.documento.arquivo.slice(0,66)}\n      empatou  : ${p.candidatosEmpatados.map(x=>x.arquivo.slice(0,58)).join(' | ')}`);
  }
  if(PER==='02.2026'){
    const alvo=L.find(l=>/MACPONTA/i.test(l.entidade)&&Math.abs(l.valor-1320000)<0.01);
    const par=[...r.pares,...r.paresVizinhos].find(p=>p.lancamento===alvo);
    console.log('\n  --- MACPONTA 02.2026 ---');
    console.log('  par:',par?par.documento.arquivo:'SEM PAR');
    console.log('  irmaos:',par&&par.irmaos?par.irmaos.map(x=>x.arquivo).join(' | '):'(nenhum)');
    console.log('  empatado:',par?!!par.empatado:'-');
    console.log('');
  }
}
console.log(`TOTAL  ${String(tp).padStart(5)}  ${String(tb).padStart(11)}  ${String(tl).padStart(9)}  ${String(ti).padStart(6)}  ${String(te).padStart(9)}`);
console.log('\n=== exemplos de IRMAOS agrupados ===');
for(const e of exIrmaos)console.log('  ',e);
console.log('\n=== exemplos de EMPATE ===');
for(const e of exEmp)console.log('  ',e);
process.exit(0);
