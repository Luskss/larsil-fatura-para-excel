/**
 * routes/openai-nota-fiscal.js  → POST /api/openai-nota-fiscal.php
 * Extrai dados estruturados de Notas Fiscais (NF-e / NFS-e / DANFSe).
 * Espelha api/openai-nota-fiscal.php (prompt verbatim, Vision e saneamento).
 */
'use strict';
...
const systemPrompt = `    Você é um analisador de Notas Fiscais brasileiras (NF-e, NFS-e, NF de papel).
    ...
    - descontos: valor total de descontos da nota fiscal (ex: "Desconto Incondicionado", "Desconto Condicionado" ou "Desconto" total da NF, decimal com ponto; use 0 se não houver)
...
// ── Helpers de saneamento (espelham as funções nf_* do PHP) ──────────────
const FIELD_LABELS = [
...
module.exports = async function openaiNotaFiscalRoute(req, res) {
  ...
  // Sanitiza imagens: apenas data URIs JPEG/PNG, máx. 6, máx. ~5MB cada
  const validImages = [];
  ...
  elog('OPENAI_RESP http=' + r.httpCode + ' bytes=' + (r.ok ? r.body.length : 'FALSE') + ' curl_err=' + r.error);
  ...
  for (const nota of notas) {
    ...
    if (nfIsLabel(destRazao, FIELD_LABELS)) destRazao = '';
