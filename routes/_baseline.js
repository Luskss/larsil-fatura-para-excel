...
const COLUNAS_OBRIGATORIAS = ['ORIG', 'NF', 'ENTIDADE', 'VL_TOTAL_CAB'];
const MAX_LINHAS_BUSCA_CABECALHO = 200;
function acharCabecalho(wb) {
    const tentativas = [];
    for (const sheetName of wb.SheetNames) {
        const rows = XLSX.utils.sheet_to_json(wb.Sheets[sheetName], { header: 1 });
        const limite = Math.min(rows.length, MAX_LINHAS_BUSCA_CABECALHO);
        for (let i = 0; i < limite; i++) {
            const header = (rows[i] || []).map(c => norm(c));
            if (COLUNAS_OBRIGATORIAS.every(c => header.includes(c))) {
                return { sheetName, rows, header, headerRow: i };
            }
        }
        tentativas.push(`${sheetName} (${rows.length} linhas)`);
    }
    throw new Error('Planilha sem cabeçalho reconhecível. Abas: ' + tentativas.join('; '));
}
...
function lerPlanilhaIndexada(planilhaPath) {
    ...
    const { sheetName, rows, header, headerRow } = acharCabecalho(wb);
    ...
    if (porPeriodo.size === 0) throw new Error('Planilha lida mas nenhum lançamento ORIG=CP encontrado.');
    planilhaCache.porVal     = porVal;
    console.log(`[comparar-notas] planilha reindexada (aba "${sheetName}", ${rows.length} linhas, ${porPeriodo.size} períodos, ${porNF.size} NFs)`);
