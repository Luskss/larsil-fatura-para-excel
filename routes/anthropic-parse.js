/**
 * routes/anthropic-parse.js → usado por ai-dispatch quando aiProvider=anthropic
 * Analisa faturas de cartão via Claude (mesma lógica de openai-parse.js).
 */
'use strict';
...
function elog(msg) { console.error('[anthropic-parse] ' + msg); }
...
module.exports = async function anthropicParseRoute(req, res) {
  ...
  if (content === '') {
    return res.status(502).json({ success: false, message: 'Resposta vazia da Anthropic.' });
  }
  ...
  if (!parsed || typeof parsed !== 'object') {
    return res.status(502).json({ success: false, message: 'Resposta da Anthropic não é JSON válido.', detail: content.slice(0, 500) });
  }
